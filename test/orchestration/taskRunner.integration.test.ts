...
describe('TaskRunner', () => {
  ...
  it('assesses the task beside the running attempt, and puts the snapshot on its telemetry', async () => {
    ...
    // `typecheck` is configured but this task plans nothing, so the model's `strong` is capped.
    expect(a.dimensions.verifiability).toMatchObject({ value: 'weak', from: 'rule' });
