...
- **Plan usage cards sit above the table** — the same numbers as Claude Code's `/usage`: the 5-hour session, the 7-day week, any model-scoped week (e.g. *Weekly Fable*), and extra-usage credits once any have been spent. Each card has the percent, a bar (blue, yellow from 70%, orange from 90%), and a *Resets in* countdown that ticks locally. They are read once a minute (`agentWrangler.usagePollIntervalSeconds`) from `GET /api/oauth/usage` with the login token Claude Code stored at sign-in (macOS Keychain item *Claude Code-credentials*, else `~/.claude/.credentials.json`); the token is only ever read — Claude Code owns refreshing it, so a rejected token shows as a stale note until Claude Code next talks to the API. If a read fails the last good numbers stay up, marked *stale*, with a refresh button. Turn the cards off with `agentWrangler.showUsage`.
